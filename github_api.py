...
async def graphql_request(query: str, access_token: str):
    data = {
        "query": query
    }
...
async def get_project_id(project_num: int):
    query = f"query {{user(login: \"{owner}\") {{projectV2(number: {project_num}){{id}}}}}}"
    res = await graphql_request(query, access_token)
    res_data = await res.json()
    return res_data["data"]["user"]["projectV2"]["id"]


async def add_draft_item_to_project(project_num: int, title: str, body: str | None = None):
    project_id = await get_project_id(project_num)

    query = f"mutation {{addProjectV2DraftIssue(input: {{projectId: \"{project_id}\" title: \"{title}\" body: \"{body}\"}}) {{projectItem {{id}} }} }}"

    return await graphql_request(query, access_token)
